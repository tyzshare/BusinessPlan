...
InitJSPlumb.prototype.initNodes = function(node, position) {
    ...
    if (position === "left") {
        DynamicAnchors.map(anchor => this.jsp.addEndpoint(node, anEndpoint, {anchor: "Right"}));
    } else {
        DynamicAnchors_Right.map(anchor => this.jsp.addEndpoint(node, anEndpoint, {anchor: "Left"}));
    }
